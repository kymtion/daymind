    ...
    func handleAuthorization(_ authResults: ASAuthorization) {
        if let appleIDCredential = authResults.credential as? ASAuthorizationAppleIDCredential {
            ...
            guard let identityTokenData = appleIDCredential.identityToken,
                ...
                self.error = NSError(domain: "LoginViewModel", code: -1, userInfo: [NSLocalizedDescriptionKey: "Unable to retrieve identity token"])
                print("Error: Unable to retrieve identity token")
                return
            }
